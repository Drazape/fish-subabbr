...
Packages should use the *RegExp* [switch](./README.md#Switches) whenever the same Sub-Command is being abbreviated more than once in the same *Base Command* (different *Initial Args*) so that users can create *Sub-Command* Abbreviations with multiple *Base Commands* easily, without having to parse RegExp themselves. [^regex-workaround]
